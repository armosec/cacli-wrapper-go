...
// ================================================================================================
func (cacli *Cacli) K8SPEList() ([]string, error) {
	args := []string{}
	peList := []string{}

	args = append(args, "k8s")
	args = append(args, "posture-exceptions")
	args = append(args, "list")
	res, err := cacli.runCacliCommandRepeat(args, false, time.Duration(3)*time.Minute)
	if err == nil {
		err = json.Unmarshal(res, &peList)
	}
	return peList, err
}

func (cacli *Cacli) K8SPEGet(name, framework, control, cluster, namespace string) ([]armotypes.PostureExceptionPolicy, error) {
	...
	if framework != "" {
		args = append(args, "--framework-name")
		args = append(args, framework)
	} else if control != "" {
		args = append(args, "--control-name")
		args = append(args, control)
	}

	wtReceive, err := cacli.runCacliCommandRepeat(args, true, time.Duration(2)*time.Minute)
	if err == nil {
		err = json.Unmarshal(wtReceive, &pe)
	}
	return pe, err
}
...
func (cacli *Cacli) ClusterConfigGet(name, cluster, scope string) (armotypes.CustomerConfig, error) {
	cc := armotypes.CustomerConfig{}
	args := []string{}
	args = append(args, "cluster")
	args = append(args, "customer-configuration")
	args = append(args, "get")
	if name != "" {
	...
	wtReceive, err := cacli.runCacliCommandRepeat(args, true, time.Duration(2)*time.Minute)
	if err == nil {
		err = json.Unmarshal(wtReceive, &cc)
	}
	return cc, err
}
...
// ============================= K8S POSTURE ======================================================
// ================================================================================================
func (cacli *Cacli) K8SPOSTURECreate(framework, cluster, namespace, wlid string) error {
	args := []string{}
	...
	_, err := cacli.runCacliCommandRepeat(args, false, time.Duration(3)*time.Minute)
	if err != nil {
		return err
	}
	return nil
...
// DEPRECATED !!! The function name is not following the convention
func (cacli *Cacli) RunPostureScan(framework, cluster string) error {
	/*
		Please follow the convention:
		cacli <verb1> <verb2> <command>
		The function name should look like:
		VERB1VERB2Command (verb should be upper-case. command - first letter upper case)
	*/
	return cacli.K8SPOSTURECreate(framework, cluster, "", "")
}
